({
    ...
    preencheTabela: function (cmp, event, helper) {
        var checkboxSelecionarTodos = document.getElementById('selecionarTodosCheckbox');
        if (checkboxSelecionarTodos) {
            checkboxSelecionarTodos.checked = false;
        }
        console.log("checkboxSelecionarTodos existe? ", checkboxSelecionarTodos);
        cmp.set("v.produtosSelecionados", '');
        //EXIBE SPINNER DE CARREGAMENTO
        helper.showSpinner(cmp);
        var isChecked = cmp.find("btnApenasDestinados").get("v.checked");

        console.log('isChecked: ', isChecked);
        var filtroDestinados = '';
        if (isChecked) {
            filtroDestinados = 'AND Quantidade_destinada__c > 0';
        }
        else {
            filtroDestinados = 'AND Quantidade_destinada__c = 0'
        }
...
<div title='Clique para expandir' data-status='"+ Status + "' data-idProduto='" + idProduto + "' data-idItemPC='" + idItemPC + "' data-quantidadeDisponivel='" + quantidadeDisponivel + "' data-quantidade = '" + qtdTotal + "' data-quantidadeRecebida = '" + quantidadeRecebida + "' data-valorTotal = '" + valorTotal + "' data-urlImagem = '" + urlImagem + "' data-codigo='" + codigo + "' data-marca='" + marca + "' data-nomeProduto='" + produto + "' data-codigo='" + codigo + "' data-modelo='" + modelo + "' id='itemTabela' class='containerMestreBlocos itemTabela' draggable='false' style='padding-bottom: 10px;' >\
   <div style='z-index: 1; cursor: pointer' class='blocoItemPedidoVenda blocoItemPedidoVendaEvento'>\
      <div style='height: 100%;' class='row'>\
         <div style='display: flex; height: 100%; align-items: center;' class='col-md-6'>\
           <div style='padding: 0 10px;'><input type='checkbox' class='checkboxItem' data-idProduto='"+ idItemPC + "' data-action='updateSelection' style='width: 18px; height: 18px; cursor: pointer; accent-color: #1a972b;' onclick='event.stopPropagation();'></div>\
   ...
   <div id='itemTabelaSecundaria' style='background-color: rgb(245, 246, 248); box-shadow: 0 2px 8px rgb(14 19 24 / 7%); height: 90px;' class='blocoItemPedidoVenda blocoItemPedidoVendaExpand'>\
    ...
    eventsAfterPreencheTabela: function (cmp, event, helper) {
        ...
        $('.selectpicker').off('changed.bs.select').on('changed.bs.select', function (e, clickedIndex, newValue, oldValue) {
            ...
            helper.validaAtualizaStatusItem(cmp, event, helper, iDItemPc, status, oldValue)
                .then(function () {
                    helper.atualizaStatusItem(cmp, event, helper, iDItemPc, status);
                })
                .catch(function (erro) {
                    console.warn("Validação bloqueou:", erro);
                });
    ...
    mudaTodosStatus: function (cmp, event, helper, stts) {
        ...
        var recordId = cmp.get("v.recordId");
        var itensDoPedidoDeCompra = cmp.get("v.produtosSelecionados") || [];
        console.log("Itens sendo enviados para classe: ", itensDoPedidoDeCompra);
        ...
        if (itensDoPedidoDeCompra.length === 0) {
            helper.hideSpinner(cmp);
            helper.alertaErro(
                cmp, event, helper,
        ...
        helper.limparMensagensErro();

        // Busca os itens com seus status atuais
        const idsFormatados = "('" + itensDoPedidoDeCompra.join("','") + "')";
        const query = `SELECT Id, Status__c FROM Item_de_fornecedor__c WHERE Id IN ${idsFormatados}`;

        this.soql(cmp, query).then(function (itensRetornados) {
            console.log('Itens retornados da query: ', itensRetornados);

            if (!itensRetornados || itensRetornados.length === 0) {
                helper.hideSpinner(cmp);
                helper.alertaErro(
                    cmp, event, helper,
            ...
            // Array para armazenar promessas de validação
            var validacoesPromises = [];
            var itensValidados = [];
            var itensBloqueados = [];

            // Valida cada item individualmente
            itensRetornados.forEach(function (item) {
                var promessaValidacao = helper.validaAtualizaStatusItem(
                    cmp, event, helper,
                    item.Id,
                    stts,
                    item.Status__c
                ).then(function () {
                    // Se passou na validação, adiciona à lista de validados
                    itensValidados.push(item.Id);
                    return { sucesso: true, id: item.Id };
                }).catch(function (erro) {
                    // Se falhou na validação, adiciona à lista de bloqueados
                    itensBloqueados.push({
                        id: item.Id,
                        statusAtual: item.Status__c,
                        erro: erro
                    });
                    return { sucesso: false, id: item.Id, erro: erro };
                });

                validacoesPromises.push(promessaValidacao);
            });

            // Aguarda todas as validações
            Promise.all(validacoesPromises).then(function (resultados) {
                console.log('Resultados das validações:', resultados);
                console.log('Itens validados:', itensValidados);
                console.log('Itens bloqueados:', itensBloqueados);

                // Se existem itens bloqueados, exibe erro nos cards
                if (itensBloqueados.length > 0) {
                    helper.exibirErrosNosCards(
                        itensBloqueados.map(function (item) { return item.id; }),
                        "Alguns itens não puderam ser atualizados devido às regras de validação"
                    );
                }

                // Se TODOS os itens foram bloqueados
                if (itensValidados.length === 0) {
                    helper.hideSpinner(cmp);
                    helper.alertaErro(
                        cmp, event, helper,
                        "NENHUM ITEM PÔDE SER ATUALIZADO. VERIFIQUE AS REGRAS DE STATUS.",
                        "VALIDAÇÃO BLOQUEADA",
                        "error",
                        "Erro: ",
                        "sticky"
                ...
                // Se existem itens validados, prossegue com a atualização
                if (itensValidados.length > 0) {
                    // CHAMA A FUNÇÃO DO APEX QUE ATUALIZA OS ITENS VALIDADOS
                    var action = cmp.get("c.atualizaStatusItensDoPedidoDeCompra");

                    // DEFINE OS PARÂMETROS PASSADOS PARA CLASSE APEX
                    action.setParams({
                        pc: recordId,
                        stts: stts,
                        itens: itensValidados  // Envia apenas os itens que passaram na validação
                    });
                    ...
                    action.setCallback(this, function (response) {
                        ...
                        if (state === "SUCCESS") {
                            ...
                            if (resultado.sucesso) {
                                // Verifica se houve itens com falha (sucesso parcial)
                                if (resultado.itensFalha && resultado.itensFalha.length > 0) {
                                    // Exibe mensagens de erro nos cards dos itens que falharam
                                    helper.exibirErrosNosCards(resultado.itensFalha, resultado.mensagem);

                                    // Alerta com resumo - sucesso parcial
                                    var mensagemFinal = resultado.mensagem;
                                    if (itensBloqueados.length > 0) {
                                        mensagemFinal += "\n" + itensBloqueados.length + " item(ns) bloqueado(s) por validação.";
                                    }

                                    helper.alertaErro(
                                        cmp, event, helper,
                                        mensagemFinal,
                                        "ATENÇÃO: ALGUNS ITENS NÃO FORAM ATUALIZADOS",
                                        "warning",
                                        "",
                                        "dismissable"
                                    );
                                } else {
                                    // Todos os itens validados foram atualizados com sucesso

                                    var mensagemSucesso = "ALTERAÇÃO DE STATUS EM MASSA CONCLUÍDA";
                                    if (itensBloqueados.length > 0) {
                                        mensagemSucesso += "\n" + itensValidados.length + " item(ns) atualizado(s). " +
                                            itensBloqueados.length + " item(ns) bloqueado(s) por validação.";
                                    }
                                    ...
                                    helper.alertaErro(
                                        ...
                                        mensagemSucesso,
                                        "TUDO OK!",
                                        itensBloqueados.length > 0 ? "warning" : "success",
                                        "",
                                        "dismissable"
                                    );
                                ...
                                helper.alertaErro(
                                    cmp, event, helper,
                                    resultado.mensagem || "NÃO FOI POSSÍVEL PROCESSAR A ATUALIZAÇÃO",
                                    "ERRO NA ALTERAÇÃO EM MASSA",
                                    "error",
                                    "Erro: ",
                                    "sticky"
                                );
                            ...
                            helper.alertaErro(
                                cmp, event, helper,
                                "ERRO DURANTE A ALTERAÇÃO DE STATUS EM MASSA",
                            ...
                            if (errors && errors[0] && errors[0].message) {
                                ...
                                helper.alertaErro(
                                    cmp, event, helper,
                                    "ERRO DESCONHECIDO",
                                    "OCORREU UM ERRO DURANTE A ALTERAÇÃO EM MASSA",
                                    "error",
                                    "Erro: ",
                                    "sticky"
                                );
                ...
                helper.alertaErro(
                    cmp, event, helper,
                    "ERRO DURANTE A VALIDAÇÃO DOS ITENS",
                    "Erro!",
                    "error",
                    "Erro: ",
                    "sticky"
    ...
    // Nova função para exibir mensagens de erro nos cards
    exibirErrosNosCards: function (itensFalha, mensagemGeral) {
        // Percorre todos os itens que falharam
        itensFalha.forEach(function (idItemFalha) {
            // Encontra o card correspondente ao item que falhou
            var $card = $('.itemTabela[data-iditempc="' + idItemFalha + '"]');

            if ($card.length > 0) {
                // Remove mensagem de erro anterior se existir
                $card.find('.mensagem-erro-item').remove();

                // Cria a mensagem de erro
                var mensagemErro = $('<div>', {
                    class: 'mensagem-erro-item',
                    style: 'background-color: #ffebee; border-left: 4px solid #f44336; padding: 10px; margin-top: 5px; border-radius: 4px;',
                    html: '<i class="fa fa-exclamation-triangle" style="color: #f44336; margin-right: 8px;"></i>' +
                        '<strong style="color: #c62828;">Falha ao atualizar:</strong> ' +
                        '<span style="color: #d32f2f;">Não foi possível processar este item.</span>'
                });

                // Adiciona a mensagem de erro após o card principal
                $card.find('.blocoItemPedidoVenda').first().after(mensagemErro);

                // Adiciona uma borda vermelha ao card para destacar
                $card.find('.blocoItemPedidoVenda').first().css({
                    'border': '2px solid #f44336',
                    'border-radius': '4px'
                });

                // Scroll suave até o primeiro item com erro
                if (itensFalha.indexOf(idItemFalha) === 0) {
                    $('html, body').animate({
    ...
    // Nova função para limpar mensagens de erro
    limparMensagensErro: function () {
        // Remove todas as mensagens de erro
        $('.mensagem-erro-item').remove();

        // Remove as bordas vermelhas dos cards
        $('.blocoItemPedidoVenda').css({
            'border': '',
            'border-radius': ''
        });
    },
    ...
    validaAtualizaStatusItem: function (cmp, event, helper, iDItemPc, status, oldValue) {
        return new Promise(function (resolve, reject) {

            console.table([
                { nome: 'elemento selecionado', valor: iDItemPc },
            ...
            if (status === "RECEBIDO TOTAL" || status === "RECEBIDO PARCIAL" || status === "NOVO") {
                helper.alertaErro(cmp, event, helper,
                    "Não é permitido alterar para status: " + status,
                    "Alteração bloqueada!", "error", "Erro: ", "dismissible");

                helper.preencheTabela(cmp, event, helper);
                console.error("Tentativa de alteração para status bloqueado:", status);

                reject("Status bloqueado");
                return;
            }
            ...
            if (oldValue === "CANCELADO") {
                helper.alertaErro(cmp, event, helper,
                    ...
                    "Status atual: Cancelado", "error", "Erro: ", "dismissible");

                helper.preencheTabela(cmp, event, helper);
                console.error("Tentativa de alteração de pedido cancelado.");
            ...
            if (oldValue === "RECEBIDO TOTAL") {
                helper.alertaErro(cmp, event, helper,
                    "Pedidos já recebidos não podem ter status alterado",
                    "Status atual: Recebido Total", "error", "Erro: ", "dismissible");

                helper.preencheTabela(cmp, event, helper);
                console.error("Tentativa de alteração de pedido recebido.");

                reject("Recebido total");
                return;
            }
            ...
            if (oldValue === "CMP - COTAÇÃO DE COMPRA" && statusCompras) {
                helper.alertaErro(cmp, event, helper,
                    "De 'COTAÇÃO DE COMPRA' só é permitido mudar para status transitórios",
                    "Transição inválida", "error", "Erro: ", "dismissible");

                helper.preencheTabela(cmp, event, helper);
                console.error("Transição bloqueada:", oldValue);

                reject("Transição inválida");
                return;
            }

            // --- Se passou de todas validações ---
            resolve(true);
        });
    },
    ...
    atualizaStatusItem: function (cmp, event, helper, iDItemPc, status) {
        /* var iDItemPc = $(elemento).attr('data-iditempc'); */
...
"
    ...
    atualizarProdutosSelecionados: function (component) {
        var idsMarcados = [];
        var checkboxes = document.querySelectorAll('.checkboxItem:checked');

        checkboxes.forEach(function (checkbox) {
            var idProduto = checkbox.getAttribute('data-idProduto');
            if (idProduto) {
                idsMarcados.push(idProduto);
            }
        });

        component.set("v.produtosSelecionados", idsMarcados);

        console.log('Produtos selecionados atualizados:', component.get("v.produtosSelecionados"));
    }
