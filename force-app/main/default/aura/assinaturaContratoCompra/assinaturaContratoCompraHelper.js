({
    objetoConsulta: 'Contrato_de_compra__c',
    campoConsulta: 'Aprovado__c, Assinante__r.MobilePhone, Assinante__r.Name, Assinante__r.CPF__c, Assinante__r.Email, idDocAutentique__c, Fase__c, StatusAssinatura__c, Testemunha__r.CPF__c, Testemunha__r.Email, Testemunha__r.LastName, Testemunha__c, Testemunha__r.FirstName',
        ...
        .then(function (idsAutentique) {
            ...
            idsAutentique.forEach(function(resultado){
                ...
                if(resultado.hasOwnProperty('idDocAutentique__c')){
                    ...
                    /*
                    if(resultado.Aprovado__c == false){
    ...
    consultaInfos: function(cmp, event, helper){
        ...
        .then(function (idsAutentique) {
            ...
            idsAutentique.forEach(function(resultado){
                ...
                if(resultado.hasOwnProperty('Assinante__c')){
                    var idAssinante2 = resultado.Assinante__c
                    var nomeAssinante2 = resultado.Assinante__r.Name;
                    var cpfAssinante2 = resultado.Assinante__r.CPF__c;
                    var emailAssinante2 = resultado.Assinante__r.Email;
                    var celularAssinante2 = resultado.Assinante__r.MobilePhone;
                }else{
                    var idAssinante2 = ""
                    var nomeAssinante2 = "";
                    var cpfAssinante2 = "";
    ...
    enviaAssinatura: function(cmp, event, helper, recordId){
        ...
        var tipoAssinatura = $("#tipoAssinatura").val();

        //CHAMA A FUNÇÃO DO APEX QUE ADICIONA O ITEM
        var action = cmp.get("c.enviarCompra");
        
        console.log("ID CONTRATO",recordId )
        console.log("TIPO ASSINATURA", tipoAssinatura)
        
        //DEFINE OS PARÂMETROS PASSADOS PARA CLASSE APEX----
        action.setParams({
            idContrato: recordId,
            proprio: tipoAssinatura
        });
        //--------------------------------------------------
    ...
    preencheVisualizacao: function(cmp, event, helper, statusAssinatura){
        ...
        var steps = ['Aprovação jurídica', 'Assinatura do fornecedor', 'Assinatura do administrador', 'Assinaturas finalizadas']     
    ...
    consultaSignatarios: function(cmp, event, helper, fase){
        ...
        //CHAMA A FUNÇÃO
        var action = cmp.get("c.consultarCompra");
        var recordId = cmp.get("v.recordId")
    ...
    cancelaDocumento: function(cmp, event, helper){
        if (confirm("Deseja mesmo cancelar a assinatura deste documento?")) {
            ...
            //CHAMA A FUNÇÃO DO APEX QUE ADICIONA O ITEM
            var action = cmp.get("c.cancelarCompra");
            var recordId = cmp.get("v.recordId")
