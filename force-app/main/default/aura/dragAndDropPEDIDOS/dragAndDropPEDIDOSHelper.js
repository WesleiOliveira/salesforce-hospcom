({
    ...
    showSpinner: function (cmp) {
        ...
        $('#spinnerDivOrder').css("display", "flex");
    },
    //-------------------------------------------
    ...
    hideSpinner: function (cmp, event, helper) {
        if(!helper.identificadorErro){
            $('#spinnerDivOrder').css("display", "none");
        }
    },
    //--------------------------------------------
    ...
    buscaProdutos: function (cmp, event, helper, inputPesquisa, filtro) {
        ...
        helper.soql(cmp, "SELECT Pricebook2Id FROM Order WHERE id = '" + recordId + "'")
            ...
            response.forEach(function (resposta) {
                ...
                if (filtro === "null" && inputPesquisa != "all" && !usaPesquisaAvancada) {
                    var query = "SELECT Name, Total_aguardando_entrega_transicional__c, ID_do_Produto__c, URL_da_Imagem__c, Modelo__c, Qtd_Aguardando_Recebimento__c, Valor_Total__c, Description, ProductCode, Marca__c, Estoque_disponivel__c FROM Product2 WHERE IsActive = true AND (Name like '%" + inputPesquisa + "%' OR Modelo__c like '%" + inputPesquisa + "%' OR Marca__c like '%" + inputPesquisa + "%' OR ProductCode like '%" + inputPesquisa + "%') ORDER BY  LastModifiedDate  DESC LIMIT 100"
                        ...
                        var query = "SELECT Name, Total_aguardando_entrega_transicional__c, ID_do_Produto__c, URL_da_Imagem__c, Modelo__c, Qtd_Aguardando_Recebimento__c, Valor_Total__c, Description, ProductCode, Marca__c, Estoque_disponivel__c FROM Product2 WHERE AND IsActive = true AND (Name like '%" + inputPesquisa + "%' OR Modelo__c like '%" + inputPesquisa + "%' OR Marca__c like '%" + inputPesquisa + "%' OR ProductCode like '%" + inputPesquisa + "%') AND  Linha__c = '" + helper.selectLinha + "' ORDER BY Name LIMIT 100"
                            ...
                            var query = "SELECT Name, Total_aguardando_entrega_transicional__c, ID_do_Produto__c, URL_da_Imagem__c, Modelo__c, Qtd_Aguardando_Recebimento__c, Valor_Total__c, Description, ProductCode, Marca__c, Estoque_disponivel__c FROM Product2 WHERE IsActive = true AND (Name like '%" + inputPesquisa + "%' OR Modelo__c like '%" + inputPesquisa + "%' OR Marca__c like '%" + inputPesquisa + "%' OR ProductCode like '%" + inputPesquisa + "%') AND Linha__c = '" + helper.selectLinha + "' AND Family = '" + helper.selectFamilia + "' ORDER BY Name LIMIT 100"
                                ...
                                var query = "SELECT Name, Total_aguardando_entrega_transicional__c, ID_do_Produto__c, URL_da_Imagem__c, Modelo__c, Qtd_Aguardando_Recebimento__c, Valor_Total__c, Description, ProductCode, Marca__c, Estoque_disponivel__c FROM Product2 WHERE IsActive = true AND (Name like '%" + inputPesquisa + "%' OR Modelo__c like '%" + inputPesquisa + "%' OR Marca__c like '%" + inputPesquisa + "%' OR ProductCode like '%" + inputPesquisa + "%') AND Linha__c = '" + helper.selectLinha + "' AND Family = '" + helper.selectFamilia + "' AND  Tipo_do_Produto__c = '" + helper.selectTipo + "' ORDER BY Name LIMIT 100"
                                    ...
                                    //SE O INPUT DE PESQUISA FOR ALL, ISTO É, A CAIXA DE PESQUISA É VAZIA---
                                        ...
                                        if (filtro === 'null') {
                                            var query = "SELECT Name, Total_aguardando_entrega_transicional__c, ID_do_Produto__c, URL_da_Imagem__c, Modelo__c, Qtd_Aguardando_Recebimento__c, Valor_Total__c, Description, ProductCode, Marca__c, Estoque_disponivel__c FROM Product2 WHERE IsActive = true ORDER BY LastModifiedDate  DESC LIMIT 100"
                                            } else if (filtro === 'familia') {
                                                var query = "SELECT Name, Total_aguardando_entrega_transicional__c, ID_do_Produto__c, URL_da_Imagem__c, Modelo__c, Qtd_Aguardando_Recebimento__c, Valor_Total__c, Description, ProductCode, Marca__c, Estoque_disponivel__c FROM Product2 WHERE IsActive = true AND Linha__c = '" + helper.selectLinha + "' ORDER BY LastModifiedDate  DESC LIMIT 100"
                                                } else if (filtro === 'tipo') {
        ...
        .catch(function (error) {
            console.log("Erro ao recuperar o priceBookId" + error)
        })
    },
    //-----------------------------------------------------------------------
        ...
        if (confirm(textAlerta)) {
            ...
            action.setCallback(this, function (response) {
                ...
                //CASO O MÉTODO SEJA EXECUTADO DE FORMA INCOMPLETA
                    ...
                    helper.alertaErro(cmp, event, helper, "ERRO DURANTE A EXCLUSAO", "INSERÇÃO IMCOMPLETA", "error", "Erro: ", "sticky") //EXIBE UM ALERTA DE INSERÇÃO INCOMPLETA AO USUÁRIO
                } else if (state === "ERROR") {
                    var errors = response.getError();
                    if (errors && errors[0] && errors[0].message) {
                        helper.alertaErro(cmp, event, helper, errors[0].message, "ERRO AO EXCLUIR UM PRODUTO", "error", "Erro: ", "sticky")
    ...
    unificaValores: function (cmp, helper, item) {
        
        //DEFINE A VISUALIZAÇÃO DO NOME DO PRODUTO-------------------------
        var idItemPV = item.attr("data-iditemcotacao")
        
        console.log("UNIFICA VALORES", idItemPV)
        
        var action = cmp.get("c.unificaValores");
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS") {
                ...
                helper.alertaErro(cmp, event, helper, "Valores unificados", "SUCESSO", "success", "Operação concluída!", "dismissable")
            }
            
            //CASO O MÉTODO SEJA EXECUTADO DE FORMA INCOMPLETA
            else if (state === "INCOMPLETE") {
                helper.alertaErro(cmp, event, helper, "Valores não unificados", "Um erro aconteceu", "error", "Erro: ", "sticky") //EXIBE UM ALERTA DE INSERÇÃO INCOMPLETA AO USUÁRIO
            } else if (state === "ERROR") {
                var errors = response.getError();
                if (errors && errors[0] && errors[0].message) {
                    helper.alertaErro(cmp, event, helper, errors[0].message, "Valores não unificados", "error", "Erro: ", "sticky")
                    reject(errors[0].message);
                } else {
                    console.log("Erro desconhecido");
                    helper.alertaErro(cmp, event, helper, "ERRO DESCONHECIDO", "OCORREU UM ERRO DESCONHECIDO", "error", "Erro: ", "sticky")
...
<div style='flex-direction: column !important; height:20px !important;' class='divMidiaFilter itemMainCotacao divProdutosFilhos' id='divProdutosFilhos'>\
		...
		<a id='exibirProdutosTextTag'>Exibir arquivos de mídia</a>\
                            ...
                            listaTipos.forEach(function (tipoAtual) {
                                ...
                                $(item).click(function () {
                                    ...
                                    helper.preencheProdutosMkt(cmp, event, helper, categoriaPai, tipo, helper.filtroModelo)
                                })
    ...
    adicionaItemCotacao: function (cmp, event, helper, item) {
        ...
        var ultimoIndice = helper.retornaIndiceUltimoItem(cmp, event, helper)
        ...
        if(origemItem == "produtoFilho"){
            ...
            action.setCallback(this, function (response) {
                ...
                if (state === "SUCCESS") {
                    ...
                    var errors = response.getError();
                    if (errors && errors[0] && errors[0].message) {
                        helper.alertaErro(cmp, event, helper, errors[0].message, "ERRO AO INSERIR NO PEDIDO", "error", "Erro: ", "sticky")
                        reject(errors[0].message);
                    } else {
                        console.log("Erro desconhecido");
                        helper.alertaErro(cmp, event, helper, "ERRO DESCONHECIDO", "OCORREU UM ERRO DESCONHECIDO", "error", "Erro: ", "sticky")
    ...
    calculaTotais: function (cmp, event, helper) {
        ...
        $(document).find("#totalCotacao").text("TOTAL DO PEDIDO BRL " + new Intl.NumberFormat('id', { minimumFractionDigits: 2 }).format(totalCotacao))
    ...
    consultaCotacao: function (cmp, event, helper, option) {
        ...
        var itensNaoConformes = false
        var itensFilhosNaoConformes = false
        
        $("#listaCotacao").empty()
        
        //REALIZA A CONSULTA
        this.soql(cmp, "SELECT Order.Cliente_inadimplente__c, Order.TotalAmount, Order.Pedido_da_Linha_de_OPME__c, Order.Opportunity.Saldo_de_empenho__c, Ordem__c, Id, Order.Status, Product2.Valor_Total__c, Product2.Estoque_disponivel__c, Product2.Valor_de_Venda__c, UnitPrice, Quantity, Product2.StockKeepingUnit, TotalPrice, Codigo_fabricante__c, Item__c, Descricao_da_linha__c, Product2.ID_do_Produto__c, Marca__c, Modelo__c, Product2.Name, Product2.ProductCode, Product2.URL_da_Imagem__c, Item_Pai__c FROM OrderItem WHERE OrderId = '" + recordId + "' ORDER BY ordem__c")
        
        //QUANDO A SOLICITAÇÃO FOR CONCLUÍDA, FAÇA:
        .then(function (itens) {
            
            //console.log("ITENS AQUI: ", itens[0])
            
            var clienteInadimplente = itens[0].Order.Cliente_inadimplente__c
            ...
            if(itens[0].Order.Pedido_da_Linha_de_OPME__c == "SIM"){
                if(itens[0].Order.Opportunity.Saldo_de_empenho__c < itens[0].Order.TotalAmount){
                    helper.alertaErro(cmp, event, helper, "O saldo de empenho do contrato é inferior ao valor inserido neste pedido", "Alerta", "Warning", "", "sticky")
                }
            }
            ...
            itens.forEach(function (itemAtual) {
                ...
                if (itemAtual.Item_Pai__c === undefined) {
                    ...
                    //EXIBE ALERTA DE PEDIDO COM ERRO
                    if(itemAtual.Item__c == undefined){
                        helper.identificadorErro = false
                    ...
                    var totalKit = parseFloat(itemAtual.TotalPrice) + itens.filter(item => item.Item_Pai__c == itemAtual.Item__c).reduce((total, item) => total + parseFloat(item.TotalPrice), 0);
                    ...
                    itensPai.push({
                        ...
                        PrecoUnitario: itemAtual.UnitPrice,
                        ValorDeVenda: itemAtual.Product2.Valor_de_Venda__c,
                        ValorVendaKit: itemAtual.Product2.Valor_Total__c,
                        ConformidadeValor: totalKit >= itemAtual.Product2.Valor_Total__c ? true : false,
                        ConformidadeEstoque: itemAtual.Product2.Estoque_disponivel__c >= itemAtual.Quantity ? true: false,
                        StatusPedido: itemAtual.Order.Status,
                        QuantidadeEstoque: itemAtual.Product2.Estoque_disponivel__c,
                        //SubTotal: itemAtual.TotalLineAmount,
                        Desconto: 0.0,
                        Total: itemAtual.TotalPrice,
                        TotalKit: totalKit,
                        Description: itemAtual.Descricao_da_linha__c,
                        CodigoFabricante: itemAtual.Product2.ProductCode,
                        Item__c: itemAtual.Item__c,
                        Item_Pai__c: itemAtual.Item_Pai__c,
                        Marca__c: itemAtual.Marca__c,
                        Modelo__c: itemAtual.Modelo__c,
                    ...
                    itensFilho.push({
                        ...
                        PrecoUnitario: itemAtual.UnitPrice,
                        ValorDeVenda: itemAtual.Product2.Valor_de_Venda__c,
                        ConformidadeValor: itemAtual.UnitPrice >= itemAtual.Product2.Valor_de_Venda__c ? true : false, 
                        ConformidadeEstoque: itemAtual.Product2.Estoque_disponivel__c >= itemAtual.Quantity ? true: false,
                        StatusPedido: itemAtual.Order.Status,
                        //SubTotal: itemAtual.TotalLineAmount,
                        Desconto: 0.0,
            ...
            itensPai.forEach(function (itemPai) {
                //DEFINE A VISUALIZAÇÃO DO NOME DO PRODUTO-------------------------
                var nomeItem = itemPai.Name.length >= 28 ? (itemPai.Name.substring(0, 28) + "...") : itemPai.Name;
                ...
                //----------------------------------------------------------------
                var NaoConformidadeFilhos = itensFilho.filter(item => item.Item_Pai__c == numberItem).some(item => item.ConformidadeEstoque == false);
                
                //console.log(itensFilho.filter(item => item.Item_Pai__c == numberItem))
                //console.log("CONFORMIDADE ITENS FILHOS", NaoConformidadeFilhos)
                
                var statusPedido = itemPai.StatusPedido
                if(statusPedido == "Aguardando Aprovação Comercial" || statusPedido == "Aguardando Aprovação Financeira"){
                    //seta a visibilidade da barra de status da validacao
                    var statusBarValidation = 'display: true'
                    
                    //verifica se a nao conformidade é no valor
                    if(!itemPai.ConformidadeValor){
                        console.log("NAO CONFORME VALOR: ", nomeItem)
                        //seta cor vermelha no card
                        var corCard = "border: 3px solid #cc1100!important;";
                        var corVenda = "background-color: #cc1100!important"
                        itensNaoConformes = true;
                    }
                    
                    //verifica se a nao conformidade é no estoque
                    if(!itemPai.ConformidadeEstoque){
                        //seta cor vermelha no card
                        var corCard = "border: 3px solid #cc1100!important;";
                        var corEstoque = "background-color: #cc1100!important"
                        itensNaoConformes = true;
                    }
                    
                    //verifica nao conformidade nos produtos filhos
                    if(NaoConformidadeFilhos){
                        //seta cor vermelha no card
                        var corCard = "border: 3px solid #cc1100!important;";
                        var corFilhos = "background-color: #cc1100!important"
                        itensNaoConformes = true;
                    }
                    
                    //caso nao esteja tudo conforme, define cor verde em todos indicadores
                    if(itemPai.ConformidadeEstoque && itemPai.ConformidadeEstoque && !NaoConformidadeFilhos){
                        console.log(NaoConformidadeFilhos)
                        console.log("CONFORMIDADE FINAL ENTROU")
                        //seta cor verde
                        var corCard = "border: 3px solid #55cd15!important;";
                        //var corVenda = "";
                        //var corEstoque = "";
                    }
                    ...
                    var corCard = "";
                    var statusBarValidation = 'display: none';
                    var corVenda = "";
                    var corEstoque = "";
                }
                ...
                if (parseInt(numberItem) === parseInt(helper.numeroDivEmEdicao)) {
                    ...
                    var inputHtml = $("<div class='containerCotacao' data-autoupdate = '0' data-tipo='produtoPai' data-marca='" + marca + "' data-codigoHospcom='" + itemPai.codigoHospcom + "' data-codigoFabricante='" + codigo + "' data-modelo='" + modelo + "' data-name='" + itemPai.Name + "' data-image='" + imageUrl + "' data-idProduto='" + idProduto + "' data-idItemCotacao='" + idItemCotacao + "' data-item-c='" + numberItem + "'>\
<div class='itemMainCotacao' style='"+corCard+"'>\
<!-- IMAGEM DO ITEM -->\
\
<div class='divImageItem'>\
	<div class='validationBar' id='validationBar' style='"+statusBarValidation+"'>\
        <div class='validationStatus' style='"+corVenda+"' title='Valor de venda conforme'><i class='fa fa-money iconValidationStatus' aria-hidden='true' ></i><span id='valorText'>&nbsp;Valor</span></div>\
        <div class='validationStatus' style='"+corEstoque+"' title='Item disponível no estoque'><i class='fa fa-cube iconValidationStatus' aria-hidden='true' ></i><span id='valorText'>&nbsp;Estoque</span></div>\
		<div class='validationStatus' style='"+corFilhos+"' title='Produtos filhos'><i class='fa fa-child iconValidationStatus' aria-hidden='true' ></i><span id='valorText'>&nbsp;Filhos</span></div>\
    </div>\
\
<img class='imageItem' src='" + imageUrl + "'/>\
</div>\
<!-- DIVISOR --> <div style='display: flex; height: 100%; width: 4%; align-items: center; justify-content: center;'> <hr style='border-left: 1px solid #dcdcdc; margin-top: 0px; height: 90%; margin-bottom: 0; opacity: 1; background-color: #ffffff'/> </div> <!-- INFO DO ITEM --> <div style='background-color: ; width: 76%; display: flex; flex-direction: column'> <!-- NOME DO ITEM E BOTÕES DE AÇÃO --> <div style='position: relative; display: flex; align-items: center; width: 100%; height: 24%'> <div style='display: flex; align-items: center; width: 80%; height: 100%; font-size: 20px; color: #00345c; font-weight: bold;'>" + nomeItem + "</div> <div style='justify-content:center; align-items: center; display: flex; align-items: center; width: 10%; height: 100%; font-size: 20px; color: #00345c; font-weight: bold;'> <div class='buttonNumberItem' id='numberItem' style=''>#" + ordem + "</div> </div><a style='width: 10%; height: 100%;' id='buttonOptions'><div style='justify-content:center; align-items: center; display: flex; align-items: center; width: 100%; height: 100%; font-size: 20px; color: #00345c; font-weight: bold;'> <div class='buttonNumberItem' style='background-color: #f4f4f4!important; border: 0px'> <i class='fa fa-sort-desc' style='color: #00345c; font-size: 18px;' aria-hidden='true'></i> </div> </div></a>\
<!-- MENU OPTIONS ITEM -->\
<div style='display: none; flex-direction: column' id='menuOptionsId' class='menuOptions'>\
<div id='optionClose' style='cursor: pointer; width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
...
<div id='optionShowDetails' style='cursor: pointer; width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
...
<div id='optionUnificaValores' style='cursor: pointer; width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
<div style='width: 80%; height: 100%; color: #a0bb31; display: flex; padding-right: 5px; justify-content: right; align-items: center; '> Exibir Detalhes</div>\
<div style='width: 20%; height: 100%; color: #a0bb31; display: flex; align-items: center; justify-content: center; text-align: center'>\
<i class='fa fa-th-large' style='width: 100%; height: 100%; display: flex; font-size: 20px; align-items:center; text-align:center' aria-hidden='true'></i>\
</div>\
</div>\
<div id='optionUpdate' style='cursor: pointer; cursor: pointer; width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
...
<div id='optionCloseOptions' style='cursor: pointer; width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
...
<!-- MODELO DO ITEM --> <div style='width: 100%; height: auto; font-size: 12px; color: #00345c; display: flex; align-items: center'>" + modCodMarca + "</div> <!-- DIVISOR --> <div style='display: flex; height: 4%; height: 8%; align-items: center;'> <hr style='border-top: 1px solid #dcdcdc; margin-top: 2px; width: 95%; margin-bottom: 0; opacity: 1; background-color: #ffffff'/> </div> <!-- DESCRIÇÃO --> <div style='width: 100%; height: auto'> <div style='font-size: 11px; color: #00345e; height: auto; width: 100%; display: flex; justify-content: center; align-items: center; border-radius: 5px;'><textarea id='textAreaDesc' value='Descrição' class='customTextArea' id='multiliner' name='multiliner'></textarea> </div> </div> <!-- VALORES --> <div style='column-gap: 2px; padding-bottom: 1px; padding-top: 1px; width: 100%; height: 26%; font-size: 12px; color: #A0BB31; display: flex'> <div style='border-radius: 5px; display: flex; justify-content: center; flex-direction: column; height: 100%; width: 40%; background-color: #a0bb31'> <div style='height: 50%; font-weight: 700;    font-size: 10px; color: #00345c; display: flex; justify-content: center; align-items: center'>VALOR UNITÁRIO (BRL)</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='valorUnitario' name='lname' value='BRL 300.000,00'/> </div> </div> <div style='border-radius: 5px; display: flex; justify-content: center; flex-direction: column; height: 100%; width: 20%; background-color: #a0bb31'> <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'>QNTD</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='quantidade' name='lname' value='1'/> </div> </div> <div style='border-radius: 5px; display: flex; justify-content: center; flex-direction: column; height: 100%; width: 40%; background-color: #a0bb31'> <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'>Total (BRL)</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input disabled='true' style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='totalItemPai' name='lname' value='BRL 300.000,00'/> </div> </div> </div> <!-- TOTAL DA MAIN UNITY --> <div style='background-color: grown; width: 100%; height: 20%'> <div style='font-weight: 700; color: white; background-color: #00345c; height: 100%; width: 100%; display: flex; justify-content: center; align-items: center; border-radius: 7px;' id='totalKit'> TOTAL: BRL 303.000,00 </div> </div> </div> </div> <div id='divProdutosFilhos' class='itemMainCotacao divProdutosFilhos' style='flex-direction: column!important; height: 300px!important;'> <div id='produtosFilhosDrop' class='produtosFilhosDrop' style='padding-top: 5px; display: flex; width: 100%; height: 95%; overflow: auto; flex-direction: column; align-items: flex-end;'> </div> <div id='exibirProdutosText' style='display: flex; width: 100%; height: 5%; justify-content: center; align-items: center'> <a id='exibirProdutosTextTag'>Exibir acessórios</a> </div> </div> </div>")
                    } else {
                        //CRIA A VARIÁVEL COM O ITEM QUE SERÁ ADICIONADO-------------------
                        var inputHtml = $("<div class='containerCotacao' data-autoupdate = '0' data-tipo='produtoPai' data-marca='" + marca + "' data-codigoHospcom='" + itemPai.codigoHospcom + "' data-codigoFabricante='" + codigo + "' data-modelo='" + modelo + "' data-name='" + itemPai.Name + "' data-image='" + imageUrl + "' data-idProduto='" + idProduto + "' data-idItemCotacao='" + idItemCotacao + "' data-item-c='" + numberItem + "'>\
<div class='itemMainCotacao' style='"+corCard+"'>\
...
</div><a style='width: 10%; height: 100%;' id='buttonOptions'>\
...
<div id='optionUnificaValores' style='cursor: pointer; width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
<div style='width: 80%; height: 100%; color: #a0bb31; display: flex; padding-right: 5px; justify-content: right; align-items: center; '> Unifica Valores</div>\
<div style='width: 20%; height: 100%; color: #a0bb31; display: flex; align-items: center; justify-content: center; text-align: center'>\
<i class='fa fa-money' style='width: 100%; height: 100%; display: flex; font-size: 20px; align-items:center; text-align:center' aria-hidden='true'></i>\
</div>\
</div>\
<div id='optionUpdate' style='cursor: pointer; cursor: pointer; width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
<div style='width: 80%; height: 100%; color: #a0bb31; display: flex; padding-right: 5px; justify-content: right; align-items: center; '> Auto Update</div>\
...
<!-- MODELO DO ITEM --> <div style='width: 100%; height: auto; font-size: 12px; color: #00345c; display: flex; align-items: center'>" + modCodMarca + "</div> <!-- DIVISOR --> <div style='display: flex; height: 4%; height: 8%; align-items: center;'> <hr style='border-top: 1px solid #dcdcdc; margin-top: 2px; width: 95%; margin-bottom: 0; opacity: 1; background-color: #ffffff'/> </div> <!-- DESCRIÇÃO --> <div style='width: 100%; height: auto'> <div style='font-size: 11px; color: #00345e; height: auto; width: 100%; display: flex; justify-content: center; align-items: center; border-radius: 5px;'><textarea id='textAreaDesc' value='Descrição' class='customTextArea' id='multiliner' name='multiliner'></textarea> </div> </div> <!-- VALORES --> <div style='column-gap: 2px; padding-bottom: 1px; padding-top: 1px; width: 100%; height: 26%; font-size: 12px; color: #A0BB31; display: flex'> <div style='border-radius: 5px; display: flex; justify-content: center; flex-direction: column; height: 100%; width: 40%; background-color: #a0bb31'> <div style='height: 50%; font-weight: 700;    font-size: 10px; color: #00345c; display: flex; justify-content: center; align-items: center'>VALOR UNITÁRIO (BRL)</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='valorUnitario' name='lname' value='BRL 300.000,00'/> </div> </div> <div style='border-radius: 5px; display: flex; justify-content: center; flex-direction: column; height: 100%; width: 20%; background-color: #a0bb31'> <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'>QNTD</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='quantidade' name='lname' value='1'/> </div> </div> <div style='border-radius: 5px; display: flex; justify-content: center; flex-direction: column; height: 100%; width: 40%; background-color: #a0bb31'> <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'>Total (BRL)</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input disabled='true' style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='totalItemPai' name='lname' value='BRL 300.000,00'/> </div> </div> </div> <!-- TOTAL DA MAIN UNITY --> <div style='background-color: grown; width: 100%; height: 20%'> <div style='font-weight: 700; color: white; background-color: #00345c; height: 100%; width: 100%; display: flex; justify-content: center; align-items: center; border-radius: 7px;' id='totalKit'> TOTAL: BRL 303.000,00 </div> </div> </div> </div> <div id='divProdutosFilhos' class='itemMainCotacao divProdutosFilhos' style='flex-direction: column!important;'> <div id='produtosFilhosDrop' class='produtosFilhosDrop' style='padding-top: 5px; display: flex; width: 100%; height: 95%; overflow: auto; flex-direction: column; align-items: flex-end;'> </div> <div id='exibirProdutosText' style='display: flex; width: 100%; height: 5%; justify-content: center; align-items: center'> <a id='exibirProdutosTextTag'>Exibir acessórios</a> </div> </div> </div>")
                        }
                ...
                inputHtml.find("#optionClose, #optionShowDetails, #optionCloseOptions, #optionUpdate, #optionUnificaValores").hover(function () {
                ...
                //EXIBE DETALHES DO ITEM--------------------------------------------
                inputHtml.find("#optionUnificaValores").click(function () {
                    helper.unificaValores(cmp, helper, inputHtml)
                })
                //------------------------------------------------------------------
...
<input type='checkbox' id='autoUpdate' checked>\
                ...
                inputHtml.find("#valorUnitario, #quantidade, #desconto, #textAreaDesc").change(function () {
            ...
            console.log("ITENS FILHOS", itensFilho)
            ...
            itensFilho.forEach(function (itemFilho) {
                ...
                var itemPai = itemFilho.Item_Pai__c
                const itemDesejado = itensPai.find(obj => obj.Item__c == itemPai);
                console.log("ITEM DESEJADO", itemDesejado)
                console.log("ITEM PAI", itemPai)
                
                if (!itemDesejado) {
                    helper.identificadorErro = false
                    helper.alertaErro(cmp, event, helper, "UM OU MAIS PRODUTOS FILHOS ESTÃO SEM O VÍNCULO COM O PRODUTO PAI", "ERRO DE VÍNCULO NO PRODUTO ", "error", "", "sticky")
                }
                ...
                var statusPedido = itemFilho.StatusPedido
                if(statusPedido == "Aguardando Aprovação Comercial" || statusPedido == "Aguardando Aprovação Financeira"){
                    //seta a visibilidade da barra de status da validacao
                    
                    //verifica se a nao conformidade é no estoque
                    if(!itemFilho.ConformidadeEstoque){
                        //seta cor vermelha no card
                        var corCard = "border: 3px solid #cc1100!important;";
                        //var corEstoque = "background-color: #cc1100!important"
                        itensNaoConformes = true;
                                ...
                                <div class='itemMainCotacao' style='"+corCard+"'>\
                                    ...
                                    <div style='width: 76%; display: flex; flex-direction: column'>\
                                        ...
                                        <div style='position: relative; display: flex; align-items: center; width: 100%; height: 29%'>\
                                            ...
                                            <div id='buttonOptions' style='justify-content:center; align-items: center; display: flex; align-items: center; width: 10%; height: 100%; font-size: 20px; color: #00345c; font-weight: bold;'>\
                                                <div class='buttonNumberItem' style='background-color: #f4f4f4!important; border: 0px'>\
                                                    <i class='fa fa-sort-desc' style='color: #00345c; font-size: 18px;' aria-hidden='true'></i>\
                                                </div>\
                                            </div>\
                                            ...
                                            <div style='display: none; flex-direction: column' class='menuOptions' id='menuOptionsId'>\
                                                <div id='optionClose' style='width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
                                                    <div style='width: 80%; height: 100%; color: red; display: flex; padding-right: 5px; justify-content: right; align-items: center; '>Remover</div>\
                                                    <div style='width: 20%; height: 100%; color: red; display: flex; align-items: center; justify-content: center; text-align: center'> <i class='fa fa-times-circle' style='width: 100%; height: 100%; display: flex; font-size: 20px; align-items:center; text-align:center' aria-hidden='true'></i>\
                                                    </div>\
                                                </div>\
                                        ...
                                        <div style='width: 100%; height: auto'>\
                                            <div style='font-size: 11px; color: #00345e; height: auto; width: 100%; display: flex; justify-content: center; align-items: center; border-radius: 5px;'>\
                                                <textarea value='Descrição' class='customTextArea' id='textAreaDesc' name='multiliner'></textarea>\
                                            </div>\
                                        </div>\
                                        <!-- VALORES -->\
                                        <div style='column-gap: 2px; padding-bottom: 1px; padding-top: 1px; width: 100%; height: 31%; font-size: 12px; color: #A0BB31; display: flex'>\
                                            <div style='border-radius: 5px; display: flex; justify-content: center; flex-direction: column; height: 100%; width: 40%; background-color: #a0bb31'>\
                                                <div style='height: 50%; font-weight: 700; color: #00345c; font-size: 10px; display: flex; justify-content: center; align-items: center'>VALOR UNITÁRIO (BRL)</div>\
                                                    <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'>\
                                                        <input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='valorUnitario' name='lname' value=''/>\
                                                    </div>\
                                                </div>\
                                                <div style='border-radius: 5px; display: flex; justify-content: center; flex-direction: column; height: 100%; width: 20%; background-color: #a0bb31'>\
                                                    <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'>QNTD</div>\
                                                    <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'>\
                                                        <input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='quantidade' name='lname' value='1'/>\
                                                    </div>\
                                                </div>\
                                                <div style='border-radius: 5px; display: flex; justify-content: center; flex-direction: column; height: 100%; width: 40%; background-color: #a0bb31'>\
                                                    <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'>Total (BRL)</div>\
                                                    <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'>\
                                                        <input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='totalItem' name='lname' value=''/>\
                                                    </div>\
                                                </div>\
                                            </div>\
                                        </div>\
            ...
            //EVENTOS DOS INPUTS---------------------------------------
            $("input").filter("#valorUnitario").mask('000.000.000.000.000,00', { reverse: true }); //DEFINE AS MASCARAS NOS INPUTS DE VALORES
            ...
            if(itensNaoConformes){
                helper.alertaErro(cmp, event, helper, "Alguns itens do pedido estão abaixo do valor de venda ou não possuem estoque. Recomenda-se analisar os produtos detacados em vermelho antes da aprovação.", "SUGESTÃO DE APROVAÇÃO:", "warning", "", "sticky")
            }
