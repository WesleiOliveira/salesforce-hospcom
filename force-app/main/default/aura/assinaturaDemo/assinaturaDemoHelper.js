({
    ...
    verificaEnvio: function(cmp, event, helper){
        ...
        .then(function (idsAutentique) {
            ...
            idsAutentique.forEach(function(resultado){
                ...
                if(String(resultado.RecordTypeId) == String(helper.tipoRegistroDemo)){
                    //console.log("é demo")
        ...
        .then(function (idsAutentique) {
            ...
            idsAutentique.forEach(function(resultado){
                ...
                try {
                    ...
                    var nomeCliente = resultado.ShipToContact.FirstName + " " + resultado.ShipToContact.LastName;
                    var emailCliente = resultado.ShipToContact.Email;
                    var cpfCliente = resultado.ShipToContact.CPF__c;
                }catch(err) {
                    //Exibe Spinner de carregamento
                    helper.hideSpinner(cmp);
                    alert("Dados para assinatura digital inválidos ou incompletos")
                }
    ...
    enviaAssinatura: function(cmp, event, helper, recordId){
        ...
        var action = cmp.get("c.enviarDemo");
    ...
    preencheVisualizacao: function(cmp, event, helper, statusAssinatura){
        ...
        var steps = ['Assinatura do cliente', 'Aprovação financeira', 'Assinatura do administrador e do vendedor', 'Assinaturas finalizadas']     
    ...
    consultaSignatarios: function(cmp, event, helper, fase){
        ...
        //CHAMA A FUNÇÃO
        var action = cmp.get("c.consultarDemo");
        var recordId = cmp.get("v.recordId")
 ...
 downloadDocumento: function(cmp, event, helper){
    ...
    cancelaDocumento: function(cmp, event, helper){
        if (confirm("Deseja mesmo cancelar a assinatura deste documento?")) {
            ...
            //CHAMA A FUNÇÃO DO APEX QUE ADICIONA O ITEM
            var action = cmp.get("c.cancelarDemo");
            var recordId = cmp.get("v.recordId")
