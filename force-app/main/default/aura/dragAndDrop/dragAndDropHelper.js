({
    ...
    buscaProdutos: function (cmp, event, helper, inputPesquisa, filtro) {
        ...
        helper.soql(cmp, "SELECT opportunity.pricebook2id FROM quote WHERE id = '" + recordId + "'")
        .then(function (response) {
            response.forEach(function (resposta) {
                ...
                var pricebook2id = resposta.Opportunity.Pricebook2Id;
                                                ...
                                                var query = "SELECT Name, Total_aguardando_entrega_transicional__c, ID_do_Produto__c, URL_da_Imagem__c, Modelo__c, Qtd_Aguardando_Recebimento__c, Valor_Total__c, Description, ProductCode, Marca__c, Estoque_disponivel__c FROM Product2 WHERE id in (SELECT product2id FROM priceBookEntry WHERE pricebook2id = '" + pricebook2id + "') AND IsActive = true AND Linha__c = '" + helper.selectLinha + "' ORDER BY LastModifiedDate  DESC LIMIT 100"
                                                    ...
                                                    var query = "SELECT Name, Total_aguardando_entrega_transicional__c, ID_do_Produto__c, URL_da_Imagem__c, Modelo__c, Qtd_Aguardando_Recebimento__c, Valor_Total__c, Description, ProductCode, Marca__c, Estoque_disponivel__c FROM Product2 WHERE id in (SELECT product2id FROM priceBookEntry WHERE pricebook2id = '" + pricebook2id + "') AND IsActive = true AND Linha__c = '" + helper.selectLinha + "' AND Family = '" + helper.selectFamilia + "' ORDER BY LastModifiedDate  DESC LIMIT 100"
                                                        ...
                                                        var query = "SELECT Name, Total_aguardando_entrega_transicional__c, ID_do_Produto__c, URL_da_Imagem__c, Modelo__c, Qtd_Aguardando_Recebimento__c, Valor_Total__c, Description, ProductCode, Marca__c, Estoque_disponivel__c FROM Product2 WHERE id in (SELECT product2id FROM priceBookEntry WHERE pricebook2id = '" + pricebook2id + "') AND IsActive = true AND Linha__c = '" + helper.selectLinha + "' AND Family = '" + helper.selectFamilia + "' AND  Tipo_do_Produto__c = '" + helper.selectTipo + "' ORDER BY LastModifiedDate  DESC LIMIT 100"
                                                            ...
                                                            var query = "SELECT Name, Total_aguardando_entrega_transicional__c, Tipo_do_Produto__c, ID_do_Produto__c, URL_da_Imagem__c, Modelo__c, Qtd_Aguardando_Recebimento__c, Valor_Total__c, Description, ProductCode, Marca__c, Estoque_disponivel__c FROM Product2 WHERE id in (SELECT product2id FROM priceBookEntry WHERE pricebook2id = '" + pricebook2id + "') AND IsActive = true AND Linha__c = '" + helper.selectLinha + "' AND Family = '" + helper.selectFamilia + "' AND  Tipo_do_Produto__c = '" + helper.selectTipo + "' AND Marca__c = '" + helper.selectMarca + "' ORDER BY LastModifiedDate  DESC LIMIT 100"
                                                            }
                                    } /*else {
                    ...
                    produtos.forEach(function (produtoAtual) {
                        ...
                        if (produtoAtual.Estoque_disponivel__c == 0 || produtoAtual.Estoque_disponivel__c == undefined || produtoAtual.Estoque_disponivel__c == "undefined") {
                            ...
                            if (produtoAtual.Qtd_Aguardando_Recebimento__c >= 1) {
                                ...
                                if (aguardandoReceber == undefined || aguardandoReceber == "undefined" || aguardandoReceber == 0) {
    ...
    ativaDragAndDropFilhos: function (cmp, item, helper) {

        //INSTANCIA O DRAG-AND-DROP NA DIV DOS PRODUTOS FILHOS----------------------
        var element = $(item).find('#produtosFilhosDrop')[0] //ESTE SELETOR É EQUIVALENTE A document.getElementById('produtosFilhosDrop');
		//var elements = document.querySelectorAll('.produtoFilho');
...
\
			<div style='width: 85%; height: auto; display: flex; flex-direction: column; align-items: center'>\
                ...
                <div id='listaFiltrosMkt' class='listaFiltrosMkt'>\
                <!-- APPEND DOS TIPOS -->\
                </div>\
...
/*<!--ARQUIVOS DE MIDIA -->\
<div style='flex-direction: column !important; height:20px !important;' class='divMidiaFilter itemMainCotacao divProdutosFilhos' id='divProdutosFilhos'>\
...
<a id='exibirProdutosTextTag'>Exibir arquivos de mídia</a>\
                            ...
                            //LOOP QUE PERCORRE TODOS OS PRODUTOS ENCONTRADOS, ITERANDO SOBRE PRODUTOATUAL---------------------------------------
                            produtos.forEach(function (produtoAtual) {
                                //console.log(produtoAtual.Parte__r.Description)
                                //VERIFICA SE O PRODUTO POSSÚI DESCRIÇÃO---
                            ...
                            var sortable = Sortable.create(el, {
                                ...
                                filter: ".customAUnderline, .containerHeaderResult, .divMidiaFilter", //DESATIVA A ORDENÇÃO NA DIV QUE EXIBE OS FILTROS DE UMA CATEGORIA FINAL
...
")
            ...
            Array.from(document.getElementsByClassName("midiaButton")).forEach(e => e.addEventListener('click',function(){
                var id = e.id.split(' ')
                var url = (id[0] == 'midiaButtonDownload' ? 'https://hospcomhospitalar.force.com/Sales/sfc/servlet.shepherd/version/download/' + id[1] : (id[0] == 'midiaButtonShow' ? id[1] : ''))
    ...
    adicionaItemCotacao: function (cmp, event, helper, item) {
        //Exibe Spinner de carregamento
        helper.showSpinner(cmp);
        console.log("COMECOU A ADICIONAR")
		var inicio = new Date();
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS") {
                ...
                console.log("Tempo de execução: " + tempoDeExecucao + " ms");
                ...
                //helper.alertaErro(cmp, event, helper, "O Produto foi inserido com sucesso ao pedido!", "SUCESSO", "success", "Operação concluída!", "dismissable")
                ...
                var errors = response.getError();
                if (errors && errors[0] && errors[0].message) {
                    helper.alertaErro(cmp, event, helper, errors[0].message, "ERRO AO INSERIR NA COTAÇÃO", "error", "Erro: ", "sticky")
                    reject(errors[0].message);
                } else {
                    console.log("Erro desconhecido");
                    helper.alertaErro(cmp, event, helper, "ERRO DESCONHECIDO", "OCORREU UM ERRO DESCONHECIDO, VERIFIQUE OS DADOS DA CONTA DO CLIENTE", "error", "Erro: ", "sticky")
    ...
    adicionaItemFilhoCotacao: function (cmp, helper, item) {
        ...
        console.log("COMECOU A ADICIONAR FILHO")
        var inicio = new Date();
        ...
        //QUANDO O DATA-ITEM-C == UNDEFINED SIGNIFICA QUE O ITEM NÃO É UM ITEM PAI
        if ($(item).attr('data-item-c') === undefined) {
            //console.log($(item).attr('data-tipo'))
            if ($(item).attr('data-tipo') === undefined) {
    ...
    ativaDragAndDrop: function (cmp, event, helper) {
        ...
        var sortable = Sortable.create(el, {
            ...
            onUpdate: function (evt) {
                ...
                helper.atualizaListaItens(cmp, evt.item, helper, 0)
                //----------------------------------------------------------------------------------------
            },
    ...
    atualizaListaItens: function (cmp, event, helper, option) {
        //Exibe Spinner de carregamento
        helper.showSpinner(cmp);
        ...
        $('#listaCotacao').children().each(function () {
            ...
            if (idProduto != undefined) {
                ...
                var productId = $(this).attr('data-idproduto');
                var resultadoEncontrado = helper.idsItensCotacao.find(c => c.Product2Id == productId)
                //console.log("encontrei 1 produto com undefined, id: ")
                //console.log(resultadoEncontrado)

                listaItens.push({ idProduto: resultadoEncontrado.Id, posicaoProduto: posicaoProduto })
            }
        });
        ...
        //CHAMA A FUNÇÃO DO APEX QUE ORDENA O ITEM
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS") {
                ...
                if (errors && errors[0] && errors[0].message) {
                    ...
                    console.log("Erro desconhecido");
                    helper.alertaErro(cmp, event, helper, "ERRO DESCONHECIDO", "OCORREU UM ERRO DESCONHECIDO DURANTE A ORDENAÇÃO DA LISTA, VERIFIQUE OS DADOS DA CONTA DO CLIENTE", "error", "Erro: ", "sticky")
                    reject("Erro desconhecido");
                }
            }
        });
    ...
    atualizaItem: function (cmp, event, helper, item) {
        ...
        var valorUnitario = parseFloat(String($(item).closest(".containerCotacao").find("#valorUnitario").val()).replace('BRL ', '').replaceAll(".", "").replaceAll(",", "."));
        var quantidade = parseFloat($(item).closest(".containerCotacao").find("#quantidade").val().replace(".", "").replace(",", "."))
        var subtotal = parseFloat(String($(item).closest(".containerCotacao").find("#subtotal").val()).replace('BRL ', '').replace(".", "").replace(",", "."))
        ...
        if(autoUpdate == 1){
            helper.alertaErro(cmp, event, helper, "O Auto Update está ativo para esse item. As quantidades serão atualizadas também nos produtos filhos. Caso seja necessário, desative-o no menu de opções do item.", "Atenção", "informational", "", "Dismissible")
        }
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS") {
                ...
                if (errors && errors[0] && errors[0].message) {
                    ...
                    console.log("Erro desconhecido");
                    helper.alertaErro(cmp, event, helper, "ERRO DESCONHECIDO", "OCORREU UM ERRO DESCONHECIDO DURANTE A ATUALIZAÇÃO DO ITEM, VERIFIQUE OS DADOS DA CONTA DO CLIENTE", "error", "Erro: ", "sticky")
                    //OCULTA SPINNER DE CARREGAMENTO---
                    helper.hideSpinner(cmp, event, helper);
                    //---------------------------------
                    reject("Erro desconhecido");
    ...
    consultaCotacao: function (cmp, event, helper, option) {
        ...
        $("#listaCotacao").empty()
		var query = "SELECT Quote.Cliente_inadimplente__c, ordem__c, Id, UnitPrice, Quantity, Product2.StockKeepingUnit, Subtotal, Discount, TotalPrice, Codigo_fabricante__c, Item__c, Descricao_da_linha__c, Product2.ID_do_Produto__c, Marca__c, Modelo__c, Product2.Name, Product2.ProductCode, Product2.URL_da_Imagem__c, Item_Pai__c FROM QuoteLineItem WHERE QuoteId = '" + recordId + "' ORDER BY ordem__c"
        console.log(query)
        ...
        this.soql(cmp, query)
            ...
            .then(function (itens) {
                ...
                var clienteInadimplente = itens[0].Quote.Cliente_inadimplente__c
                if(clienteInadimplente){
                    console.log("Cliente possui debitos")
                ...
                itens.forEach(function (itemAtual) {
                    if (itemAtual.Item_Pai__c === undefined) {
                        if(itemAtual.Item__c == undefined){
                            helper.identificadorErro = true
                            helper.alertaErro(cmp, event, helper, "UM OU MAIS PRODUTOS PAIS NÃO POSSUEM O CAMPO ITEM__C PREENCHIDOS. CORRIJA ANTES DE CONTINUAR.", "ERRO DE VÍNCULO NO PRODUTO " + itemAtual.Product2.Name, "error", "", "sticky")
                        }
                        //ADICIONA NA LISTA DE ITENS PAI
                        itensPai.push({ codigoHospcom: itemAtual.Product2.StockKeepingUnit, id: itemAtual.Id, ordem: itemAtual.ordem__c, idProduto: itemAtual.Product2.ID_do_Produto__c, Valor: itemAtual.UnitPrice, Quantidade: itemAtual.Quantity, SubTotal: itemAtual.Subtotal, Desconto: itemAtual.Discount, Total: itemAtual.TotalPrice, Description: itemAtual.Descricao_da_linha__c, CodigoFabricante: itemAtual.Product2.ProductCode, Item__c: itemAtual.Item__c, Item_Pai__c: itemAtual.Item_Pai__c, Marca__c: itemAtual.Marca__c, Modelo__c: itemAtual.Modelo__c, Name: itemAtual.Product2.Name, Image: itemAtual.Product2.URL_da_Imagem__c })
                    } else {
                        //ADICIONA NA LISTA DE ITENS FILHO
                        itensFilho.push({ codigoHospcom: itemAtual.Product2.StockKeepingUnit, id: itemAtual.Id, idProduto: itemAtual.Product2.ID_do_Produto__c, Valor: itemAtual.UnitPrice, Quantidade: itemAtual.Quantity, SubTotal: itemAtual.Subtotal, Desconto: itemAtual.Discount, Total: itemAtual.TotalPrice, Description: itemAtual.Descricao_da_linha__c, CodigoFabricante: itemAtual.Product2.ProductCode, Item__c: itemAtual.Item__c, Item_Pai__c: itemAtual.Item_Pai__c, Marca__c: itemAtual.Marca__c, Modelo__c: itemAtual.Modelo__c, Name: itemAtual.Product2.Name, Image: itemAtual.Product2.URL_da_Imagem__c })
                    }
                })
                //---------------------------------------------------------------------
                ...
                itensPai.forEach(function (itemPai) {
                    ...
                    var descricao = itemPai.Description;
                    ...
                    var desconto = itemPai.Desconto.toFixed(1) + "%";
                    var total = new Intl.NumberFormat('id', { minimumFractionDigits: 2 }).format(itemPai.Total);
                    //----------------------------------------------------------------
                    //
                    //console.log("VALOR UNITARIO ANTES FORMAT", itemPai.Valor)
                    //console.log("VALOR UNITARIO", valorUnitario)

                    if (parseInt(numberItem) === parseInt(helper.numeroDivEmEdicao)) {
...
<div style='position: relative; display: flex; align-items: center; width: 100%; height: 24%; justify-content: space-between'>\
<div class='nomeDoItem'>" + nomeItem + "</div>\
<div class='containerOptionsMain'>\
<div style='justify-content:center; align-items: center; display: flex; align-items: center; width: 50%; height: 100%; font-size: 20px; color: #00345c; font-weight: bold;'>\
<div class='buttonNumberItem' id='numberItem'>#" + ordem + "</div>\
</div>\
<a style='width: 50%; height: 100%;' id='buttonOptions'>\
...
<div id='optionClose' style='width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
...
<div id='optionRemoveAcessorios' style='width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
<div style='width: 80%; height: 100%; color: red; display: flex; padding-right: 5px; justify-content: right; align-items: center; '> Remover Acessórios</div>\
<div style='width: 20%; height: 100%; color: red; display: flex; align-items: center; justify-content: center; text-align: center'>\
<i class='fa fa-times-circle' style='width: 100%; height: 100%; display: flex; font-size: 20px; align-items:center; text-align:center' aria-hidden='true'></i>\
</div>\
</div>\
<div id='optionShowDetails' style='width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
...
<div id='optionUpdate' style='cursor: pointer; width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
...
<div id='optionCloseOptions' style='width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
...
<!-- MODELO DO ITEM --> <div style='width: 100%; height: auto; font-size: 12px; color: #00345c; display: flex; align-items: center'> " + modCodMarca + "</div> <!-- DIVISOR --> <div style='display: flex; height: 4%; height: 8%; align-items: center;'> <hr style='border-top: 1px solid #dcdcdc; margin-top: 2px; width: 95%; margin-bottom: 0; opacity: 1; background-color: #ffffff' /> </div> <!-- DESCRIÇÃO --> <div class='cont-descricao'> <div style='font-size: 11px; color: #00345e; height: auto; width: 100%; display: flex; justify-content: center; align-items: center; border-radius: 5px;'> <textarea id='textAreaDesc' value='Descrição' class='customTextArea' id='multiliner' name='multiliner'></textarea> </div> </div> <!-- VALORES --> <div style='column-gap: 2px; padding-bottom: 1px; padding-top: 1px; width: 100%; height: 26%; font-size: 12px; color: #A0BB31; display: flex'> <div style='' class='valoresItens'> <div style='height: 50%; font-weight: 700; font-size: 10px; color: #00345c; display: flex; justify-content: center; align-items: center'> Valor Unitário (BRL)</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='valorUnitario' name='lname' value='BRL 300.000,00' /> </div> </div> <div class='valoresItens' class='valoresItens' style='width:15%;'> <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'> QNTD</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='quantidade' name='lname' value='1' /> </div> </div> <div class='valoresItens'> <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'> Subtotal (BRL)</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='subtotal' name='lname' value='BRL 300.000,00' /> </div> </div> <div class='valoresItens' class='valoresItens' style='width:15%;'> <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'> DESC.</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='desconto' name='lname' value='0.00%' /> </div> </div> <div class='valoresItens' style='width:15%;'> <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'> Total (BRL)</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input disabled='true' style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='totalItemPai' name='lname' value='BRL 300.000,00' /> </div> </div> </div> <!-- TOTAL DA MAIN UNITY --> <div style='background-color: grown; width: 100%; height: 20%'> <div class='totalKit' id='totalKit'> TOTAL: BRL 303.000,00 </div> </div> </div> </div> <div id='divProdutosFilhos' class='itemMainCotacao divProdutosFilhos' style='flex-direction: column!important; height: 300px!important;'> <div id='produtosFilhosDrop' class='produtosFilhosDrop' style='padding-top: 5px; display: flex; width: 100%; height: 95%; overflow: auto; flex-direction: column; align-items: flex-end;'> </div> <div id='exibirProdutosText' style='display: flex; width: 100%; height: 5%; justify-content: center; align-items: center'> <a id='exibirProdutosTextTag'>Exibir acessórios</a> </div> </div> </div>")
                        } else {
                            //CRIA A VARIÁVEL COM O ITEM QUE SERÁ ADICIONADO-------------------
                            var inputHtml = $("\
<div class='containerCotacao' data-autoupdate = '0' data-marca='" + marca + "' data-codigoHospcom='" + itemPai.codigoHospcom + "' data-codigoFabricante='" + codigo + "' data-modelo='" + modelo + "' data-name='" + itemPai.Name + "' data-image='" + imageUrl + "' data-idProduto='" + idProduto + "' data-idItemCotacao='" + idItemCotacao + "' data-item-c='" + numberItem + "'>\
...
<textarea id='textAreaDesc' value='Descrição' class='customTextArea' id='multiliner' name='multiliner'></textarea>\
</div>\
</div>\
<!-- VALORES -->\
<div style='column-gap: 2px; padding-bottom: 1px; padding-top: 1px; width: 100%; height: 26%; font-size: 12px; color: #A0BB31; display: flex'>\
<div style='' class='valoresItens'>\
...
<input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='valorUnitario' name='lname' value='BRL 300.000,00' />\
</div>\
</div>\
<div class='valoresItens' class='valoresItens' style='width:15%;'> <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'> QNTD</div>\
<div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'>\
<input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='quantidade' name='lname' value='1' /> </div> </div> <div class='valoresItens'> <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'> Subtotal (BRL)</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='subtotal' name='lname' value='BRL 300.000,00' /> </div> </div> <div class='valoresItens' class='valoresItens' style='width:15%;'> <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'> DESC.</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='desconto' name='lname' value='0.00%' /> </div> </div> <div class='valoresItens' style='width:15%;'> <div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'> Total (BRL)</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'> <input disabled='true' style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='totalItemPai' name='lname' value='BRL 300.000,00' /> </div> </div> </div> <!-- TOTAL DA MAIN UNITY --> <div style='background-color: grown; width: 100%; height: 20%'> <div class='totalKit' id='totalKit'> TOTAL: BRL 303.000,00 </div> </div> </div> </div> <div id='divProdutosFilhos' class='itemMainCotacao divProdutosFilhos' style='flex-direction: column!important;'> <div id='produtosFilhosDrop' class='produtosFilhosDrop' style='padding-top: 5px; display: flex; width: 100%; height: 95%; overflow: auto; flex-direction: column; align-items: flex-end;'> </div> <div id='exibirProdutosText' style='display: flex; width: 100%; height: 5%; justify-content: center; align-items: center'> <a id='exibirProdutosTextTag'>Exibir acessórios</a> </div> </div> </div>")
                            }
                    ...
                    inputHtml.find("#totalItemPai").val(total)
                    //----------------------------------------------------------------


                    //EXPANDE OU RECOLHE O TEXTAREA------------------------------------
                    inputHtml.find("#textAreaDesc").click(function () {
                        //console.log("click em textarea expand")
                        $(this).css({ "height": "200px" });
                    })
                    inputHtml.find("#textAreaDesc").focusout(function () {
                        $(this).css({ "height": "20px" });
                    })
                    //------------------------------------------------------------------

                    //EXPANDE E RECOLHE A DIV COM OS PRODUTOS FILHOS--------------------
                    inputHtml.find("#exibirProdutosText").click(function () {
                        ...
                        if ($(this).closest('#divProdutosFilhos').css('height') == '300px') {
                            ...
                            $(this).children('#exibirProdutosTextTag').text('Exibir acessórios'); //ok
                            ...
                            $(this).children('#exibirProdutosTextTag').text('Ocultar acessórios');
                    ...
                    inputHtml.find("#buttonOptions").click(function () {
                        if ($(this).parent().parent().find("#menuOptionsId").css('display') == 'none') {
                            $(this).parent().parent().find("#menuOptionsId").css({ "display": "flex" });
                        } else {
                            $(this).parent().parent().find("#menuOptionsId").css({ "display": "none" });
                        }
                    })
                    //------------------------------------------------------------------
                    ...
                    //REMOVE ITEM DA COTAÇÃO--------------------------------------------
                    inputHtml.find("#optionRemoveAcessorios").click(function () {
                        helper.removeTodosAcessorios(cmp, event, helper, this)
                    })
                    //------------------------------------------------------------------
                    

                    //ALTERA CORES COM HOVER NA OPÇÃO FECHAR----------------------------
                    inputHtml.find("#optionClose, #optionShowDetails, #optionCloseOptions, #optionUpdate, #optionRemoveAcessorios").hover(function () {
                        $(this).css({ "background-color": "#f4f4f4", "border-radius": "15px" })
                    ...
                    inputHtml.find("#optionUpdate").click(function () {
                    ...
                    helper.ativaDragAndDrop(cmp, event, helper)
                    helper.ativaDragAndDropFilhos(cmp, inputHtml, helper)
                ...
                itensFilho.forEach(function (itemFilho) {
                    ...
                    var modCodMarca = modelo + " | " + marca + ": " + codigo
                    var idProduto = itemFilho.idProduto
                    //-----------------------------------------------------------------

                    //DEFINE DESCRIÇÃO DO PRODUTO, IMAGEM E NÚMERO DO ITEM-------------
                    var descricao = itemFilho.Description;
                    ...
                    $('#listaCotacao').children().each(function () {
                        ...
                        if ($(this).attr('data-item-c') === String(itemFilho.Item_Pai__c)) {
                            ...
                            //CRIA A VARIÁVEL COM O ITEM QUE SERÁ ADICIONADO-------------------
                            var inputHtml = $("\
<div class='containerCotacao produtoFilho' id='produtoFilho' data-autoupdate = '0' data-tipo='produtoFilho' data-idProduto='" + idProduto + "' data-marca='" + marca + "' data-codigoHospcom='" + itemFilho.codigoHospcom + "' data-codigoFabricante='" + codigo + "' data-modelo='" + modelo + "' data-name='" + itemFilho.Name + "' data-image='" + imageUrl + "' data-idItemCotacao='" + idItemCotacao + "' style='width: 95%!important; margin-right: 5px;'>\
...
<div style='display: flex; height: 100%; width: 4%; align-items: center; justify-content: center;'>\
<hr style='border-left: 1px solid #dcdcdc; margin-top: 0px; height: 90%; margin-bottom: 0; opacity: 1; background-color: #ffffff'/> </div>\
<!-- INFO DO ITEM -->\
<div style='background-color: ; width: 76%; display: flex; flex-direction: column'>\
<!-- NOME DO ITEM E BOTÕES DE AÇÃO -->\
<div style='position: relative; display: flex; align-items: center; width: 100%; height: 29%; justify-content: space-between;'>\
<div class='nomeDoItem'>" + nomeItem + "</div>\
...
<div class='' style=''>#1</div> </div>\
<div id='buttonOptions' class='buttonOptionsFilho'>\
<div class='buttonNumberItem' style='background-color: #f4f4f4!important; border: 0px'>\
<i class='fa fa-sort-desc' style='color: #00345c; font-size: 18px;' aria-hidden='true'></i>\
</div> </div> <!-- MENU OPTIONS ITEM -->\
<div style='display: none; flex-direction: column' class='menuOptions' id='menuOptionsId'>\
<div id='optionClose' class='optionClose' style='width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'>\
<div style='width: 80%; height: 100%; color: red; display: flex; padding-right: 5px; justify-content: right; align-items: center; '>Remover</div>\
<div style='width: 20%; height: 100%; color: red; display: flex; align-items: center; justify-content: center; text-align: center'>\
<i class='fa fa-times-circle' style='width: 100%; height: 100%; display: flex; font-size: 20px; align-items:center; text-align:center' aria-hidden='true'></i> </div> </div>\
<div id='optionShowDetails' class='optionShowDetails' style='width: 100%; height:33.3%; display: flex; justify-content: right; align-items: center'> <div style='width: 80%; height: 100%; color: #a0bb31; display: flex; padding-right: 5px; justify-content: right; align-items: center; '>Exibir Detalhes</div>\
...
</div> <div id='optionCloseOptions' class='optionCloseOptionsFilho'> <div style='width: 80%; height: 100%; color: #a0bb31; display: flex; padding-right: 5px; justify-content: right; align-items: center; '>Fechar</div>\
<div style='width: 20%; height: 100%; color: #a0bb31; display: flex; align-items: center; justify-content: center; text-align: center'> <i class='fa fa-minus-circle' style='width: 100%; height: 100%; display: flex; font-size: 20px; align-items:center; text-align:center' aria-hidden='true'></i> </div> </div> </div> </div>\
<!-- MODELO DO ITEM -->\
<div style='width: 90%; height: 15%; font-size: 12px; color: #00345c; display: flex; align-items: center'>" + modCodMarca + "</div> <!-- DIVISOR --> <div style='display: flex; height: 4%; height: 13%; align-items: center;'>\
<hr style='border-top: 1px solid #dcdcdc; margin-top: 2px; width: 95%; margin-bottom: 0; opacity: 1; background-color: #ffffff'/> </div> <!-- DESCRIÇÃO --> <div style='width: 100%; height: auto'> <div style='font-size: 11px; color: #00345e; height: auto; width: 100%; display: flex; justify-content: center; align-items: center; border-radius: 5px;'>\
<textarea value='"+descricao+"' class='customTextArea textAreaDescFilho' id='textAreaDesc' name='multiliner'></textarea> </div> </div> <!-- VALORES --> <div style='column-gap: 2px; padding-bottom: 1px; padding-top: 1px; width: 100%; height: 31%; font-size: 12px; color: #A0BB31; display: flex'> <div class = 'valoresItens'>\
...
<div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'>\
<input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='valorUnitario' name='lname' value='"+valorUnitario+"'/> </div> </div> <div class='valoresItens'  style='width:15%;'>\
<div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'>QNTD</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'>\
<input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='quantidade' name='lname' value='"+quantidade+"'/> </div> </div> <div class = 'valoresItens'>\
<div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'>Subtotal (BRL)</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'>\
<input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='subtotal' name='lname' value='"+subtotal+"'/> </div> </div> <div  class='valoresItens'class='valoresItens'>\
<div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'>DESC.</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'>\
<input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' id='desconto' name='lname' value='"+desconto+"'/> </div> </div> <div class = 'valoresItens'>\
<div style='height: 50%; font-weight: 700; color: #00345c; display: flex; justify-content: center; align-items: center'>Total (BRL)</div> <div style='height: 50%; font-weight: 600; color: white; display: flex; justify-content: center; align-items: center'>\
<input style='text-align: center; background-color: rgb(0, 0, 0, 0); border: 0px solid;width: 100%;height: 100%;' type='text' disabled='true' id='totalItem' name='lname' value='"+total+"'/> </div> </div> </div> </div> </div> </div>")
                ...
                helper.eventsAfterAddFilhos(cmp, event, helper);
                ...
                $("input, textarea").not("#default, #autoUpdate").hover(function () {
                ...
                $(".containerCotacao").mouseover(function () {
                    $(this).children(".itemMainCotacao").css("box-shadow", "rgba(0, 0, 0, 0.08) 0px 4px 12px")
                })
                $(".containerCotacao").mouseout(function () {
                    $(this).children(".itemMainCotacao").css("box-shadow", "")
                })
                
                $('#listaCotacao').scrollTop(helper.posicaoDivCotacao)
                
                if (option === 1) {
                    helper.atualizaListaItens(cmp, event, helper, 0)
                } else {
    ...
    //----------------------------------------------------------
    eventsAfterAddFilhos: function(cmp, event, helper){
        
        //EXPANDE E RECOLHE O MENU DE OPÇÕES DO ITEM-----------------------
        $(".buttonOptionsFilho").off().on( "click", function() {
            if ($(this).parent().find("#menuOptionsId").css('display') == 'none') {
                $(this).parent().find("#menuOptionsId").css({ "display": "flex" });
            } else {
                $(this).parent().find("#menuOptionsId").css({ "display": "none" });
        ...
        //ALTERA CORES COM HOVER NO MENU DE OPÇÕES DO ITEM------------------
        $(".buttonOptionsFilho").hover(function () {
            $(this).find(".buttonNumberItem").css({ "background-color": "#cfcfcf" })
        ...
        //EXPANDE OU RECOLHE O TEXTAREA------------------------------------
        $(".textAreaDescFilho").off().on( "click", function() {
            $(this).css({ "height": "200px" });
        })
        $(".textAreaDescFilho").focusout(function () {
            $(this).css({ "height": "20px" });
        })
        //-----------------------------------------------------------------
        
        //RECOLHE O MENU CASO CLICK EM FECHAR------------------------------
        $(".optionCloseOptionsFilho").off().on( "click", function() {
            $(this).parent().css({ "display": "none" });
        })
        //------------------------------------------------------------------
        
        //EXIBE DETALHES DO ITEM--------------------------------------------
        $(".optionShowDetails").off().on( "click", function() {
            var inputHtml = $(this).parents('#produtoFilho')
            helper.exibeDetalhesItem(cmp, helper, inputHtml)
        })
        //------------------------------------------------------------------
        ...
        //ALTERA CORES COM HOVER NA OPÇÃO FECHAR----------------------------
        $(".optionClose, .optionShowDetails, .optionCloseOptionsFilho").hover(function () {
            $(this).css({ "background-color": "#f4f4f4", "border-radius": "15px" })
        }, function () {
            $(this).css("background-color", "#ffffff");
        });
    ...
    removeTodosAcessorios: function(cmp, event, helper, elemento){
        console.log("REMOVENDO TODOS ACESSORIOS");
        
        var textAlerta = "Deseja mesmo remover todos acessórios desse item?"
        var textAlertaErro = "ITENS REMOVIDOS"
        ...
        if (confirm(textAlerta)) {
            ...
            var recordId = helper.retornaRecorId(cmp, event, helper) //VARIÁVEL QUE ARMAZENA O RECORDID DO REGISTRO
            var itemC = $(elemento).closest(".containerCotacao").attr('data-item-c');	//VARIÁVEL QUE ARMAZENA O ID DO PRODUTO A SER ADICIONADO
            
            var action = cmp.get("c.removerFilhos"); //DEFINE A CHAMADA DO MÉTODO APEX QUE IRÁ INSERIR O ITEM FILHO
