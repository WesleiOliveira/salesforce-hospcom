({
	...
	mainFunction : function(cmp, event, helper) {
        ...
        const query = "SELECT BillingState, ShippingState, BillingStreet, BillingCity, BillingPostalCode, ShippingStreet, ShippingCity, ShippingPostalCode FROM Order WHERE Id = '"+recordId+"'";
        ...
        this.soql(cmp, query).then(value => {
            intgButton.addEventListener("click", function integFunction() {
                ...
                fetch('http://integracao.hospcom.net/sales-order/' + recordId).then((response) => {
